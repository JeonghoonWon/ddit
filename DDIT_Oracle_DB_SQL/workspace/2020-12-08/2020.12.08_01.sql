...
예) 2005년 1~6월 모든 거래처별 매출현황을 조회하시오.
    Alias 거래처코드, 거래처명, 매출액   -- 모든이 붙으면 아우터 조인
 (일반 OUTER JOIN)
...
(SUBQUERY)
  SELECT A.BUYER_ID AS 거래처코드, 
         A.BUYER_NAME AS 거래처명, 
         SUM(B.CART_QTY*C.PROD_PRICE) AS 매출액
    FROM BUYER A,(2005년 1~6월 거래처별 매출액 - 모두가 빠졌기때문에 내부조인이다) B
   WHERE A.BUYER_ID=B.거래처코드(+); 
   
   2005 1~6월 거래처별 매출액-내부조인
...
예) 2005년 모든 제품별 매입/매출을 조회하시오
    Alias 상품코드, 상품명, 매입수량, 매출수량, 매입금액, 매출금액
    SUBQUERY 를 사용할 것
    SELECT A.PROD_ID AS 상품코드, 
           ...
           AS 매입금액, 
           AS 매출금액
      FROM PROD A, (2005년 제품별 매입집계) B, (2005년 제품별 매출집계) C
     WHERE A.PROD_ID=B.() (+)
       AND A.PROD_ID=C.() (+);
