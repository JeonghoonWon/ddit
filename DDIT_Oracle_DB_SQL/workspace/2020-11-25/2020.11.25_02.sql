2020-11-25-02)
 4. MOD(n,c) REMAINDER(n,c)
  ...
  REMAINDER : n - c * ROUND(n/c)
    ** FLOOR(n) : n과 같거나 작은쪽에서 제일 큰 정수(n을 초과하지 않는 최대 정수)
    ...
    REMAINDER(11,3) : 11 - 3*ROUND(10/3)
                      11 - 3*ROUND(3.66666) -- 소숫점 첫번째 자리에서 반올림 일어남.
                      11 - 3*4 => 11-12 => -1
  ...
  5. FLOOR(n), CEIL(n)
    ...
    --n이 정수면 같거나 실수면 n보다 큰 가장 작은 정수
            . 소숫점이 허용되지 않는 컬럼에 정수 데이터가 입력 된 경우 소숫점을 무조건 
            반올림하여 정수자료만 저장하는 경우 주로 사용(급여, 세금, 등의 계산 항목에 
            주로 사용)
 ...
 SELECT FLOOR (10), FLOOR(10.234), CEIL(10), CEIL(10.001) FROM DUAL;
  ...
  - min 과 max 사이를 b개의 구간으로 나누었을 경우 주어진 수n이 어느 구간에 속하는지를 반환
  
  예) 회원테이블에서 회원들이 보유한 마일리지를 1~10000사이를 구간으로 나누고 
    ...
    예) 급여 2000~30000 사이의 값을 3개의 구간으로 나눌때 각 사원의 급여가 속한 구간값을 구하고, 그 값이
        1인경우 '낮은 급여', 2인경우 '중간 급여', 3인경우 '고 임금' 이라는 메시지를 비고란에 출력하시오.
