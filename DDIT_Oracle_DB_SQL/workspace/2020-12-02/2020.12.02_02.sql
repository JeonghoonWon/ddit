...
3.SELF JOIN
 ...
 (SUB QUERY) -- 하나의 SQL 문 안에 포함되어 있는 또 다른 SQL문이 있는것
   SELECT A.MEM_ID AS 회원번호, 
          A.MEM_NAME AS 회원명, 
          A.MEM_MILEAGE AS 마일리지,
          B. MILE AS "T001회원의 마일리지"
   ...
   SELECT MEM_ID AS 회원번호, 
          ...
          MEM_MILEAGE AS 마일리지
     FROM MEMBER
     WHERE MEM_MILEAGE > (SELECT MEM_MILEAGE  --괄호부터 실행됨
                       FROM MEMBER 
                       WHERE UPPER(MEM_ID) = 'T001') 
                       
     ORDER BY 3 DESC;
 
  예) '마르죠' 거래처와 같은 지역에 주소지를 두고앴는 거래처를 조회하시오.
  Alias 거래처코드, 거래처명, 주소, 담당자 
  ...
  예) 장바구니테이블에서 't001' 회원에게 판매한 2005년월별 판매현황을
      조회하시오.
      Alias 회원명, 주민번호, 월, 구매액합계
      SELECT MEM_NAME AS 회원명, 
             MEM_REGNO1주민번호, 월, 구매액합계
      FROM MEMBER 
