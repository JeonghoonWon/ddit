2020-12-04_01)서브쿼리
 ...
 - 사용되는 위치에 따라 :   일반서브쿼리(Select 절에 사용), 
                        인라인 서브쿼리(From 절에 사용),
                        중첩서브쿼리(where 절에 사용)로 구분
 - 반환하는 행/열의 수에 따라 : 단일행/단일열, 다중행/단일열, 다중행/다중열 등으로 구분되며 이는 
    사용하는 연산자에 의해 구별됨
    
   1. 연관성없는 서브쿼리
   - 메인쿼리와 서브쿼리사이에 조인이 발생되지 않는 서브쿼리
  예) 사원테이블에서 평균급여보다 많은 급여를 받는 사원정보(사원번호, 사원명, 부서코드, 급여)조회
  -- 서브쿼리 사용시, 메인과 서브가 어떤걸로 할지 정해야함
 ...
 예) 부모부서코드(PARENT_ID)가 NULL 인 부서에 소속된 사원정보를 조회하시오.
    ...
    ** 부서테이블에서 부서코드 60(IT) 의 부모부서코드를 NULL 로 변경하시오.
    UPDATE DEPARTMENTS
       SET PARENT_ID = NULL
    ...
    SELECT A.EMPLOYEE_ID AS 사원번호, 
     ...
     WHERE EXISTS (SELECT * 
                     FROM DEPARTMENTS B
                    WHERE B.PARENT_ID IS NULL -- );여기까지 출력하면 107개 출력됨
   ...
   Alias 사원번호, 사원명, 부서코드, 부서명 (SUBQUERY 사용)
   SELECT A.EMPLOYEE_ID AS 사원번호,
          (SELECT B.EMP_NAME
             FROM EMPLOYEES B
  ...
  예제) 상품테이블에서 서브쿼리를 사용하여 'P300' 번대의 상품의
   ...
   예) 사원들의 평균급여를 계산하여 평균급여보다 더 많은 급여를 받는 사원들이 소속된
       부서코드와 부서명을 출력하시오(SUBQUERY 사용)
        ...
        (서브쿼리1) : 사원테이블 평균급여보다 더 많은 급여를 받는 사원들이 소속된 부서
        SELECT B.DEPARTMENT_ID AS 
          FROM EMPLOYEES B
         WHERE B.SALARY > (평균급여);
     ...
     SELECT A. DEPARTMENT_ID AS 부서코드,
            A. DEPARTMENT_NAME AS 부서명
          FROM DEPARTMENTS A
         WHERE A.DEPARTMENT_ID IN (SELECT B.DEPARTMENT_ID AS 
                                    FROM EMPLOYEES B
    ...
    예) 상위부서가 90번 부서(기획부)인 사원들의 급여를 자신이 속해있는 부서의
    평균급여의 10% 에 해당하는 만큼 인상하시오.
    
    상위부서가 90번인 부서(기획부)인 부서의 평균급여
