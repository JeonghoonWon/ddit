2020-12-01_03) JOIN
 - RDB의 핵심 기능
-- 양쪽 테이블에 공통 컬럼이 반드시 있어야 함.
-- 식별 관계 : 부모의 기본키가 자식의 기본키가 되는것
--비식별 관계 : 부보의 기본키가 자식의 일반키? 가 되는것
-- 이상현상(Anomaly) : 삽입, 수정, 삭제, 연산을 수행할 때 부작용들이 발생할 수 있음
 ...
 SELECT 컬럼list
  ...
  WHERE 조인조건1
   [AND 조인조건2,...]
   [AND 일반조건,...]
   . 테이블명에는 별칭을 사용하여 동일한 컬럼명이 두개 이상의 테이블에 사용도니 경우
     소속을 구별해줌
   . 별칭을 사용하지 않는 경우 동일한 컬럼명은 '테이블명.컬럼명' 형식으로 기술해야 함.
   . 조인조건은 두 테이블 사이에 존재하는 같은 값을 갖는 컬럼간의 동등성('=')
     등을 평가하기위한 조건으로 n개의 테이블이 사용된 경우 적어도 n-1개의 조인조건이
     필요함
   . 일반조건과 조인조건의 기술 순서는 일정한 규칙이 없음
  
 1. CARTESIAN PRODUCT
 - 모든 행, 모든 열의 조합이 발생(행들은 곱한 값, 열들은 더한 값)
 - 특별한 목적이 아는 경우 사용되지 않음.
...
(ANSI 형식)
 ...
 2. EQUI JOIN (동등 조인)
  ...
  예) 상품테이블에서 2005년 6월 판매현황을 조회하시오.
  Alias 일자, 회원명, 상품명, 판매수량, 판매금액
