...
public class LprodIbatisTest {
	...
	/*		
			  ...
			  // 1) 저장할 데이터를 VO 객체에 담는다. 
			  LprodVO lvo = new LprodVO();
			  
			  lvo.setLprod_id(lprodId); lvo.setLprod_gu(lprodGu); lvo.setLprod_nm(lprodNm);
			  
			  // 2) sqlMapClient 객체 변수를 이용하여 처리할 쿼리문을 호출하여 실행한다. // 형식 )
			//  smc.insert("namespace값.id속성값", 파라미터클래스); // 반환값 : insert 성공 : null / insert
			//  실패 : 오류객체
			  Object obj = smc.insert("lprod.insertLprod",lvo); if(obj == null) {
			  System.out.println("insert 작업 성공"); }else {
			  System.out.println("insert 작업 실패"); } System.out.println("insert 작업 끝>>>>");
...
/*			
			// 2-2. update 작업
			 ...
			 // 2) 형식 = smc.update("namespace값.id속성값",파라미터 클래스);
			 // 반환값 : 성공한 레코드 수
