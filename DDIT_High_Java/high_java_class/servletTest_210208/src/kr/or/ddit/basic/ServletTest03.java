...
/*
 	...
 	2. 컨테이너는 web.xml에 정의된 url 패턴을 확인하여 어느 서블릿을 통해서 처리할 것인지 검색한다.
 		(이 떄 로딩이 안된 경우에는 해당 서블릿을 로딩한다. 최초에 로딩할 때 init()메서드를 자동으로 호출한다.)
 		(Servlet 3.0이상에서는 애노테이션(@webServlet)으로 설정 가능)
 	
 	3. Servlet Container 는 요청을 처리할 개별 쓰레드 객체를 생성하여 해당 서블릿 객체의 
 		service() 메서드를 호출한다.
...
public class ServletTest03 extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 방법 2 ==> 클라이언트의 요청방식(GET,POST등)을 구분해서
		//			직접 메서드를 호출하기
	...
	public void destroy() {
	 ...
	 this.getServletName()); System.out.println("에서 destroy()메서드 호출...");
