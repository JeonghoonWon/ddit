...
-- 카트에 있는 사람 중 MEMBER_DELETE 가 Y 인사람 을 지우고 MEMBER에서 Y 인 사람 삭제
CREATE OR REPLACE PROCEDURE MEMBERDELETEPROC(
    DELCOUNT OUT NUMBER
)
IS
    BEGIN
    DBMS_OUTPUT.PUT_LINE('삭제시작');
    SELECT COUNT(*) INTO DELCOUNT FROM MEMBER WHERE MEM_DELETE = 'Y';
    DELETE FROM CART WHERE CART_MEMBER IN (
        SELECT MEM_ID
        FROM MEMBER
        WHERE MEM_DELETE = 'Y'
    );
    DELETE FROM MEMBER  WHERE MEM_DELETE = 'Y'
    DBMS_OUTPUT.PUT_LINE('삭제종료');
...
DECLARE
 n NUMBER;
BEGIN
    SELECT COUNT(*) INTO n FROM MEMBER WHERE MEM_DELETE = 'Y';
...
EMPLOYEE_PWD VARCHAR2(40) NOT NULL,
EMPLOYEE_NAME VARCHAR2(15) NOT NULL,
EMPLOYEE_PHONE VARCHAR2(20),
EMPLOYEE_EMAIL VARCHAR2(60) NOT NULL,
EMPLOYEE_AUTHORITY VARCHAR2(20) DEFAULT 'ROLE_USER',
EMPLOYEE_PICTURE VARCHAR2(200) DEFAULT 'noimg.jpg',
CONSTRAINT PK_EMPLOYEE PRIMARY KEY(EMPLOYEE_ID));

COMMENT ON COLUMN EMPLOYEE.EMPLOYEE_ID IS '아이디';
COMMENT ON COLUMN EMPLOYEE.EMPLOYEE_PWD IS '패스워드';
...
COMMENT ON COLUMN EMPLOYEE.EMPLOYEE_AUTHORITY IS '권한';
COMMENT ON COLUMN EMPLOYEE.EMPLOYEE_PICTURE IS '사진';
